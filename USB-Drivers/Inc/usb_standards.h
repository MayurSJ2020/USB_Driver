...
typedef struct
{
	...
	void (*on_out_transfer_received)(uint8_t endpoint_number, uint16_t bcnt);
	void (*on_in_tranfer_completed)(uint8_t endpoint_number);
...
}UsbControlTranferStage;
/*
 * USB standard request pattern
 */
typedef struct
{
	uint8_t bmRequestType; //Direction of the data transfer, type of req and recipient
...
#define USB_BM_REQUEST_TYPE_RECIPEINT_INTERFACE (1<<0)
#define USB_BM_REQUEST_TYPE_RECIPEINT_ENDPOINT (2<<0)
#define USB_BM_REQUEST_TYPE_RECIPEINT_OTHER (3<<0)
